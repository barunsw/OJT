...
public class ThreadTest {
	...
	public static void main(String[] args) {
		...
		/*
		 * for (int i = 0; i < 10; i++) { TestThread t = new TestThread(i); t.start();
		 * 
		 ...
		 * new TestTimerTask(i);
		 * 
		 * //timer.schedule(t, 0, 1000L); timer.scheduleAtFixedRate(t, 0, 1000L);
		 * 
		 * LOGGER.debug(String.format("[%d] activeCount:%d", 0, Thread.activeCount()));
		 * }
		 */
		/*
		 * for (int i = 0; i < 3; i++) { TestThread2 t = new TestThread2(i); t.start();
		 * 
		 * LOGGER.debug(String.format("[%d] activeCount:%d", i, Thread.activeCount()));
		 * }
		 */
