...
public class FileAddressBookImpl extends UnicastRemoteObject implements RmiAddressBookInterface {
    ...
    private static final String CSV_FILE_PATH = "src/com/barunsw/ojt/mjg/common/address_book.csv";
    ...
    private void saveToCsv() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(CSV_FILE_PATH))) {
            for (AddressVo address : addressList) {
                writer.write(String.format("%d,%s,%d,%s,%s,%s%n",
                        address.getSeq(),
                        address.getName(),
                        address.getAge(),
            ...
            LOGGER.error("CSV 저장 중 오류 발생: {}", ioe.getMessage(), ioe);
        }
    }
