...
public class ProgressDialog extends JDialog implements PropertyChangeListener {
	...
	private void initComponent() throws Exception {
		...
		jProgressBar.setStringPainted(true);  // 진행률 문자열 표시
		
		jPanel_Main.setLayout(gridBagLayout);
		jPanel_Command.setLayout(gridBagLayout);
		
		jPanel_Main.add(jProgressBar, new GridBagConstraints(
			0, 0, 1, 1, 1.0, 1.0,
			GridBagConstraints.CENTER, GridBagConstraints.BOTH,
			new Insets(5, 5, 5, 5), 0, 0
		));
		
		jPanel_Main.add(jPanel_Command, new GridBagConstraints(
			0, 1, 1, 1, 1.0, 0.0,
			GridBagConstraints.CENTER, GridBagConstraints.BOTH,
	...
	public void setProgress(int progress) {
		jProgressBar.setValue(progress); // JProgressBar의 진행 상태 업데이트
	}
