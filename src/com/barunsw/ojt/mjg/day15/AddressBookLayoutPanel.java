...
public class AddressBookLayoutPanel extends JPanel {
    ...
    public AddressBookLayoutPanel(RmiAddressBookInterface rmiAddressBookInterface) {
    ...
    private void initData() {
        // DB or 파일에서 전체 목록 조회
    	try {
			personList = rmiAddressBookInterface.selectAddressList(new AddressVo());
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void insertData() {
		try {
			...
			// DB or 파일에 Insert
			rmiAddressBookInterface.insertAddress(onePerson);
		...
		try {
			...
			if (value instanceof AddressVo) {
				try {
					...
					rmiAddressBookInterface.updateAddress(addressVo);
				}
				catch (Exception ex) {
					LOGGER.error(ex.getMessage(), ex);
				}
			...
			rmiAddressBookInterface.updateAddress(onePerson);
	...
	private void deleteData() {
		...
		if (value instanceof AddressVo) {
			try {
				AddressVo addressVo = (AddressVo)value;
				rmiAddressBookInterface.deleteAddress(addressVo);
			}
			catch (Exception ex) {
				LOGGER.error(ex.getMessage(), ex);
			}
