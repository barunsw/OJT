...
import com.barunsw.ojt.phs.day14.MybatisPanel_jTable_Result_MouseAdapter;
...
public class C_ClientPanel extends JPanel {
	...
	private S_AddressBookInterface addressBook;
...
//	============================================================
	...
	private void initRMI() {
		try {
			...
			if (remote instanceof S_AddressBookInterface) {
				addressBook = (S_AddressBookInterface)remote;
			}
		}
		catch (Exception ex) {
	...
	private void addressListAllLoad() {
		...
		for ( int i=0 ; i<addressList.size() ; i++ ) {
			Vector<String> onePerson = new Vector<String>();
	...
	private void initComponent() throws Exception {
		...
		this.setBackground(new Color(0xC8FFFF));
...
//		5번째 줄 ===== 테이블 ===================================================
	...
	private Vector<String> inputDataLoad() {
		LOGGER.debug("inputDataLoad");

		// 입력되어있던것을 하나의 Vector객체로 반환
		Vector<String> inputData = new Vector<String>();
	...
	class action_EventListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btn_Add) {
				jButton_Add_ActionListener();
			}
			else if (e.getSource() == btn_Change) {
				jButton_Change_ActionListener();
			}
			else if (e.getSource() == btn_Delete) {
				jButton_Delete_ActionListener();
			}
		}
